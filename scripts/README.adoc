...
2. Extract build status and build URLs from CI/CD checks
3. Export to CSV file at `/tmp/maven_open_prs.csv`
...
* Build Status - CI/CD build status (SUCCESS, FAILURE, PENDING, UNKNOWN, etc.)
* Build URL - Direct link to build results
...
* Build Status - CI/CD status with clickable link to build results
